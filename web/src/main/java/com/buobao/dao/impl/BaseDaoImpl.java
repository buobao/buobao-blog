...
import com.buobao.utils.CommonUtil;
...
public abstract class BaseDaoImpl<T extends Entity, PK extends Serializable> implements BaseDao<T, PK> {
    ...
    public void clear() {
        //String hql = "delete from " + this.entityClass.getName() + " where 1=1";
        String hql = "from "+ this.entityClass.getName();
        List<NewsCrawler> newsList = this.getSession().createQuery(hql).setLockMode(this.entityClass.getName(), LockMode.UPGRADE_NOWAIT).list();
        Session session = this.sessionFactory.openSession();
        Transaction tx = session.beginTransaction();
        for (int i=0;i<newsList.size();i++){
            newsList.get(i).setState(BaseEnum.NewsStateEnum.DELETE);
